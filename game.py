...
def interactive_cli():
    print("=" * 40)
    print("   NUMBER FOOTBALL GRID")
    print("=" * 40)
    print()

    while True:
        grid = NameGrid()
        print("New game started. Add player names to the grid.\n")

        while True:
            print("  [A] Add a name")
            print("  [R] Remove a name")
            print("  [S] Show grid preview")
            print("  [D] Done - generate numbers & save PDF")
            print("  [Q] Quit")
            choice = input("\n> ").strip().upper()
            ...
            if choice == "A":
                ...
                if not name:
                    ...
                    continue
                if grid.add_name(row, col, name):
                    print(f"  Added '{name}' at row {row}, column {col}.\n")
                else:
                ...
                if not grid.is_complete():
                    print("  No names added yet. Add at least one name first.\n")
                    continue
                grid.generate_numbers()
                print("\nFinal grid:\n")
                print(grid)
                print()
                path = export_grid_to_pdf(grid)
